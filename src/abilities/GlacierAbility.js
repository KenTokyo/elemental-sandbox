...
import { MAX_SPIKES, VARIANTS, TAU, Role, _emit, _pos, _dir } from './glacier-scratch.js';
import { glacierSetup } from './glacier-setup.js';
import { glacierFx } from './glacier-fx.js';
 ...
 * `settings[element]` inside the update loop, which runs on a zero-length frame
...
export class GlacierAbility extends Ability {
  constructor(context, element = 'glacier') {
...
/**
 * Two halves of this engine live in `glacier-setup.js` and `glacier-fx.js`.
 *
 * GlacierAbility was past the 800-line rule in `AGENTS.md`. The bodies did not
 ...
 */
Object.assign(GlacierAbility.prototype, glacierSetup);
Object.assign(GlacierAbility.prototype, glacierFx);
