...
/**
 * The two shader sources of `VolumetricFireMaterial`, verbatim.
 *
 * They were the constructor's largest argument by a wide margin — 566 of the
 * file's 842 lines — and the 800-line rule in `AGENTS.md` does not care that a
 * line is GLSL. Nothing here is parametrised: these are the same strings the
 * material used to hold inline, moved with their indentation intact so that
 * "the shader did not change" stays literally true. The profile function comes
 * along because both stages interpolate it and nothing else reads it.
 */
/**
...
/** Hull sizing: the proxy strip is swept into the capsule the march fills. */
export const FIRE_VERTEX_GLSL = /* glsl */ `
...
/** The march itself: density field, black-body emission, scene-depth clipping. */
export const FIRE_FRAGMENT_GLSL = /* glsl */ `
        uniform float uTime;
        uniform float uRadius;
        uniform float uHeadSize;
